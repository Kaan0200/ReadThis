...
namespace readthis_doc_maker
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            // initilize
            StreamWriter streamOut = new StreamWriter("README.txt");
            ...
            streamOut.WriteLine(FormatHeader(dirInfos.Length, fileInfos.Length));
            streamOut.WriteLine(DoBreak());
        ...
        static string DoBreak() {
            return "***********************";
        }

        static string FormatHeader(int dirCount, int fileCount) {
            StringBuilder sb = new StringBuilder();
            sb.AppendFormat("By: {0}, created at: {1} \n", Environment.UserName, DateTime.Now);
            sb.AppendFormat("{0} dirs | {1} fldrs | {2} all", dirCount, fileCount, dirCount + fileCount);
